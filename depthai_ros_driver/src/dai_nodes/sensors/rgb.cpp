...
namespace dai_nodes
{
RGB::RGB(const std::string& daiNodeName, std::shared_ptr<rclcpp::Node> node, std::shared_ptr<dai::Pipeline> pipeline,
  ...
  : BaseNode(daiNodeName, node, pipeline)
{
  ...
  setManualFocusSrv = node->create_service<rif_msgs::srv::SetInt64>(
      ...
      rmw_qos_profile_services_default, setManualFocusCBGroup_);

  setFocusModeSrv = node->create_service<rif_msgs::srv::SetDepthAIFocusMode>(
      "~/set_focus_mode", std::bind(&RGB::setFocusModeCB, this, std::placeholders::_1, std::placeholders::_2),
      rmw_qos_profile_services_default, setManualFocusCBGroup_);

  getFocusModeSrv = node->create_service<rif_msgs::srv::GetDepthAIFocusMode>(
      "~/get_focus_mode", std::bind(&RGB::getFocusModeCB, this, std::placeholders::_1, std::placeholders::_2),
      rmw_qos_profile_services_default, setManualFocusCBGroup_);
...
void RGB::setupQueues(std::shared_ptr<dai::Device> device)
{
  ...
  {
    auto tfPrefix =
        getOpticalTFPrefix(getSocketName(static_cast<dai::CameraBoardSocket>(ph->getParam<int>("i_board_socket_id"))));
